...
- **Migraciones 043/044 (rubro_actividad_ramo + tasas de Incendio por rubro) YA APLICADAS contra Supabase real (2026-07-29)**: el filtro por `ramo_id` ya funciona a nivel de datos. El backend de la rama `sdd/incendio-tasas-por-rubro` ya exige `ramo_id` en el código — al mergear a `main`, desplegar backend+frontend juntos (sin esto, el selector de rubros rompe con 400). Falta la verificación en vivo 9.3 (cotizar rubros nuevos sin 422) con credenciales reales.
