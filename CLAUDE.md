...
**Auditoría de performance de Claude Routines — issue #83 (2026-08-02), PR #88 mergeado a `main`.** El bug 🔴 más grave que reportó el issue (`actualizarCotizacion` sin rollback ante fallo parcial) resultó ya corregido desde el commit `e9a2e11` (2026-07-28) — el issue quedó desactualizado, analizó un snapshot previo a ese fix. Aplicado contra Supabase real: migración `051_indices_ramo_id_variante_id.sql` (índices en `coberturas_catalogo.ramo_id`, `tasas_cobertura_ramo.ramo_id`, `cotizacion_ajustes.variante_id` — FKs de alto tráfico que la 033 no cubrió). De paso, al verificar con la suite completa, se encontró y corrigió que 9 archivos de test usaban `t.mock.module(specifier, { exports: {...} })` — la clave correcta en Node 24.13.0 es `namedExports` (`exports` se acepta sin error pero se ignora silenciosamente, dejando el mock vacío), lo que hacía fallar en cascada 81 tests. También se detectó que faltaba correr `npm install` en esta máquina (`exceljs` no estaba en `backend/node_modules`, solo hoisteado en el root del monorepo). Suite backend 143/143 en verde tras ambos fixes — ninguno tocó lógica de negocio. Pendiente del mismo issue: hallazgos #3 (cache de `ramos.service.js`), #5 (Puppeteer sin handler SIGTERM/SIGINT) y #6-9 (JS duplicado/módulos grandes en frontend).

**Auditoría de calidad de código — issue #84 (2026-08-02), PR #90 abierto.** Hallazgos #5 (3 archivos sin referencias: `docs/mockups/index-fx-preview.html`, `exports/tasas-actuales-cotizador-tajy.xlsx`, `frontend/shared/badge-preview.html`) y #6 (README.md/docs/ARCHITECTURE.md desactualizados sobre el redeploy del backend — ya hay CD real vía `.github/workflows/deploy-backend.yml`) resueltos. Pendiente del mismo issue, por orden de riesgo: centralizar traducción de errores Postgres (duplicada en `planes.service.js`/`roles.service.js`/`ramos.service.js`), extraer `calcularEdificioYContenido` a un helper compartido entre `mrc.calculator.js`/`incendio.calculator.js` (duplicación verbatim, mismo patrón que el bug histórico VIVIENDA/VIVIENDA FAMILIAR — mayor valor pero mayor riesgo, toca cálculo de prima en 2 ramos en producción), y la duplicación de frontend (topbar/banner/focus-trap/sidebar en 4 módulos de página).
