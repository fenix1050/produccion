...
- [ ] Calculadores `incendio.js` / `vida-ap.js` con la tasa como parámetro configurable (sin hardcodear) — RPF de Incendio (plan Maquinaria Básico) ya confirmado; falta el RPF de "Comercio Protección Total", de "Incendio - Edificio y Contenido" y de los 7 planes de Vida/AP (pendiente #10, sección 11 de PLAN_DESARROLLO.md)
...
- **RPF de MRC**: confirmado y cerrado para el plan `MULTIRRIESGO COMERCIO - NORMAL` (`mrc.calculator.js` end-to-end). Sigue sin confirmar para `COMERCIO PROTECCION TOTAL` — migración 022 lo desactivó (`activo = FALSE`) por pedido de Kevin (2026-07-13) porque es probable que quede sin uso; ya no aparece en el selector de planes. Se reactivaría si más adelante se confirma su RPF. La Prima Técnica Mínima de MRC ya no bloquea: se aplica como piso silencioso pre-IVA (`prima = MAX(prima_calculada, prima_tecnica_minima)`). Además, MRC exige mínimo 3 coberturas cotizadas: Incendio Edificio + Incendio Contenido cuentan como 2 fijas; los sublímites fijos no cuentan, por lo que el agente debe agregar al menos una cobertura adicional.
- **RPF de Incendio y Vida/AP confirmado (2026-07-13, migración 023)**: RPF plano (Contado 0% / Cobrador 1,6% / Boca 1,35% / Tarjeta 1%, igual para todos los planes, no varía por cuotas) cargado para los 2 planes de Incendio y los 7 de Vida/AP. "INCENDIO - EDIFICIO Y CONTENIDO" reactivado con prima técnica mínima Gs. 409.091, que también se aplica como piso silencioso pre-IVA y no corta la cotización por quedar debajo del mínimo. Vida/AP no maneja prima técnica mínima por ahora (decisión de Kevin, no dato pendiente). Falta todavía escribir `incendio.calculator.js` / `vida-ap.calculator.js` en sí (los datos ya están, la lógica no).
...
- **Carta Oferta de MRC en PDF**: implementada con Puppeteer y template propio; usa layout dinámico 3/3 con fallback automático al layout balanceado si la página de "Coberturas y condiciones" no entra en una hoja A4. Quedan pendientes los templates de Incendio y Vida/AP.
