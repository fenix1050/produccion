...
test('PF-3 readiness enables emission only after all required MRC fields are present', () => {
  ...
  assert.equal(readiness.mensaje_emision, 'La propuesta está lista para emitir.')
})

test('PF-3 readiness reports incomplete data in Spanish', () => {
  const readiness = evaluarReadiness({ propuesta: { draft_json: {} } })

  assert.equal(readiness.listo, false)
  assert.equal(
    readiness.mensaje_emision,
    'Complete los datos requeridos antes de emitir la Propuesta Formal.'
  )
})
