...
> **Estado del documento (2026-08-27, actualizado 2026-09-21):** planificación consolidada. PF-1 —
> Carta Oferta histórica — está implementada y verificada en QA para MRC. PF-2 para MRC (migración 069,
> API modular, UI `/propuestas/`) está implementado y, además, el listado de Propuestas Formales
> (`frontend/propuestas-listado/`, `GET /propuestas`, anulación con motivo) ya fue implementado y
> desplegado a TEST, verificado en vivo (ver `docs/ESTADO_PROYECTO.md`, entradas desde la sección 98).
> PF-3 está en curso, no sin implementación: existen renderers `mrc.js` (v1), `mrc-v2.js` y `mrc-v3.js`
> en desarrollo activo, sin fecha de cierre confirmada. El módulo completo (listado + wizard) todavía no
> llegó a producción — solo a TEST. Las decisiones marcadas como **Aprobada**
...
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| API de Propuesta           | La API vigente es `/propuestas/*` (`GET /propuestas`, `POST /propuestas/:id/anular`, endpoints del wizard). `POST /api/cotizaciones/:id/aceptar` y `GET /api/cotizaciones/:id/pdf-propuesta` siguen existiendo como ruta/stub, pero `aceptarCotizacion()` y `generarPdfPropuestaFormal()` (`backend/src/services/cotizacion.service.js`) siguen sin implementar y lanzan errores explícitos de "Fase 4 pendiente" — no reflejan la API real. |
| Autorización de esos stubs | Sin uso; la API real (`/propuestas/*`) sí deriva la autorización de `req.usuario`.                                                                                                                                                                                                                                                                                                                                                           |
