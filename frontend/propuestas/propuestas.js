...
function puedeProbarRenderErrorPdf() {
  return (
    state.usuario?.rol === 'admin' &&
    state.propuesta?.estado === 'error_pdf' &&
    state.propuesta.puede_probar_render_error_pdf === true
  )
}

async function probarRenderErrorPdf() {
  if (!puedeProbarRenderErrorPdf() || state.probePending) return

  state.probePending = true
  render()
  try {
    await api.post(`/propuestas/${state.propuesta.id}/test-render-error-pdf`, {})
    state.banner = { tipo: 'success', texto: 'Diagnóstico de render completado.' }
  } catch {
    state.banner = { tipo: 'error', texto: 'No fue posible completar el diagnóstico de render.' }
  } finally {
    state.probePending = false
    render()
  }
...
function renderSafeHtml(element, markup) {
  ...
  document.querySelectorAll('*').forEach((node) => {
    for (const attribute of [...node.attributes]) {
      ...
      if (
        name.startsWith('on') ||
        name === 'srcdoc' ||
        (['href', 'src', 'action', 'formaction'].includes(name) && !esUrlSegura(attribute.value))
      ) {
        node.removeAttribute(attribute.name)
      }
    }
  })
  element.replaceChildren(...document.body.childNodes)
}
...
function renderRenderProbeControl() {
  return `<section class="panel card pf-diagnostic"><div class="card__title">Diagnóstico</div><div class="card__body"><p>Ejecute una comprobación técnica controlada.</p><button type="button" class="btn-outline" data-action="probar-render-error-pdf" ${state.probePending ? 'disabled' : ''}>${state.probePending ? 'Ejecutando diagnóstico…' : 'Ejecutar diagnóstico'}</button></div></section>`
}
...
app.addEventListener('click', (event) => {
  ...
  if (action === 'probar-render-error-pdf') probarRenderErrorPdf()
  ...
  if (action === 'logout') {
    auth.logout().then(() => {
      ...
      if (loginUrl.origin === window.location.origin) window.location.assign(loginUrl.pathname)
    })
  }
})
