...
**Qué se hizo.** El JWT viaja en `tajy_session_v2` `HttpOnly`; la CSRF en `tajy_csrf_v2`, también `HttpOnly`. Los nombres nuevos agregan `_v2` al nombre base configurado o default, y ambas cookies son host-only (sin `Domain`). El backend ignora cookies legacy y, cuando llegan en una request, las expira con `Domain=.cotizador.lat` antes de autorizar o rechazar la sesión; login sin autenticar también las limpia. La primera request puede transportar una cookie vieja, pero no autentica con ella, por lo que la persona debe volver a iniciar sesión. `Domain` se usa exclusivamente para esos encabezados de expiración legacy; ni la emisión ni la limpieza de las cookies versionadas lo incluyen. La protección double-submit sigue comparando `X-CSRF-Token` con la cookie CSRF versionada mediante `crypto.timingSafeEqual`. `GET /auth/me` entrega el token con el usuario; `frontend/shared/api.js` conserva ambos en memoria y usa el token en mutaciones, sin leer `document.cookie`. `middleware/auth.js` no acepta `Authorization: Bearer` y exige `HS256`.
...
**Verificación histórica (antes del ajuste host-only).** 192/192 tests backend en verde. La prueba Playwright de esa sesión confirmó entonces que `tajy_session` no era accesible desde JS y `tajy_csrf` sí; ese comportamiento CSRF fue reemplazado por la entrega del token vía `/auth/me`. La configuración viva de TEST/PROD sigue sin inspeccionar ni verificar; este cambio solo define la limpieza del dominio legacy ya conocido.
