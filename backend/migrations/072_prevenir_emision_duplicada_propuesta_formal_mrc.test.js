...
const migrationUrl = new URL(
  './072_prevenir_emision_duplicada_propuesta_formal_mrc.sql',
  import.meta.url
)

test('migration 072 rejects an emitted Carta and concurrent issuance before PDF rendering can start', async () => {
...
test('migration 072 protects confirmation with the same emitted-Carta domain conflict', async () => {
  const sql = await readFile(migrationUrl, 'utf8')
...
test('migration 072 preserves trusted backend-only RPC execution', async () => {
  const sql = await readFile(migrationUrl, 'utf8')
