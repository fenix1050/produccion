...
| Empty amount goes unnoticed → agents skip it, MRC 3-coverage minimum fails | Med        | Auto-open edit mode on rows with empty amount. Static view shows the real formatted value (`Gs. 100.000.000`) once set, "—" only while genuinely empty (re-resolved, see question round #2) — an empty row reads visibly as "—", unlike a filled one |
...
2. **Amount visibility — RE-RESOLVED (Kevin, 2026-08-10, second round).** Superseded the entry above. The static (non-editing) state now shows the real stored value formatted as `Gs. 100.000.000` (via the existing `fmtGsConPrefijo` helper) when `sumaAsegurada` is set, and the placeholder dash ("—") only when it's genuinely empty. Trigger: Kevin flagged what he thought was a bug (pencil revealing "100.000.000" on a row he hadn't typed into) — root-caused as an existing saved amount being correctly revealed per the original hidden-by-default rule, not a bug. Once that was clarified, Kevin decided he actually prefers the value always visible, matching the originally-approved mockup's static text ("Gs. 100.000.000" in bold) rather than the always-"—" rule from the first round. `state.coberturasAdicionales[].sumaAsegurada` still flows unchanged into preview/calculation — same display-only relationship as before, just no longer hiding a set value.
3. **Auto-open edit mode** on a just-checked/just-selected coverage with an empty amount — confirmed, desired.
4. **Lock chrome on the add button** — confirmed, land in **both** call sites (`render-datos.js` Datos step and `render-detalle-plan.js` Detalle del plan step) in this same change. (Superseded by design.md D9: the Detalle del plan add button no longer exists as of PR #225 — lock chrome landed only in `render-datos.js`.)
