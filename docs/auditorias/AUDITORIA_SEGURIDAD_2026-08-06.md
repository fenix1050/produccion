...
| Sesión / autenticación | Cookie `tajy_session` httpOnly + `secure` en producción + `sameSite=lax` + dominio acotado; CSRF double-submit con comparación *timing-safe*; JWT `HS256` explícito; revocación server-side vía `token_version`; login con mitigación de canal lateral de tiempo (bcrypt contra hash dummy). Sin hallazgos. |
