...
The system MUST calculate the premium for plans using the `objeto_riesgo` mechanics as the sum of the rates applied to each declared risk object: Edificio, Instalaciones, Contenido Mueble y Equipos, Contenido Mercadería. For risk type "VIVIENDA FAMILIAR" the confirmed per-object rates are Edificio 0.90%, Instalaciones 0.90%, Contenido Mueble y Equipos 1.34%, Contenido Mercadería 1.34%, derived from a global rate of 2.24% (min 0.6%, max 35.48%).

#### Scenario: Premium for a fully declared Vivienda Familiar risk

- GIVEN a quote for risk type "VIVIENDA FAMILIAR" with insured sums declared for all 4 risk objects
...
#### Scenario: Vivienda Familiar without Contenido Mercadería

- GIVEN a quote for "VIVIENDA FAMILIAR" declaring only Edificio and Instalaciones (no Contenido Mueble y Equipos, no Contenido Mercadería)
- WHEN the premium is calculated
