...
const migrationUrl = new URL('./070_propuestas_formales_emision_mrc.sql', import.meta.url)
...
test('migration 070 makes MRC issuance private, atomic, numbered, and auditable', async () => {
...
test('migration 070 preserves the complete official digital-document availability clause', async () => {
  const sql = await readFile(migrationUrl, 'utf8')
...
test('migration 070 retains immutable emitted artifacts and requires annulment evidence', async () => {
  const sql = await readFile(migrationUrl, 'utf8')
