...
**Cambio `incendio-3-planes-y-moneda` implementado y verificado en vivo (2026-07-27).** 23/23 tasks completas en 4 PRs encadenados (commiteados localmente en `feature/incendio-moneda-pr3-service-integracion`, **sin pushear ni PR abierto todavía**): 3 planes nuevos de Incendio (Hipotecario, con/sin Inspección), moneda USD/Gs. por cotización, tipo de cambio con fallback, umbral de inspección. Migraciones 034-040 ya aplicadas contra Supabase real (no solo archivos locales). 97/97 tests backend en verde. Detalle completo, incluyendo un bug real encontrado y corregido en la verificación (mismatch de nombre `'VIVIENDA FAMILIAR'` vs `'VIVIENDA'` en la tasa de tipo de riesgo), en `docs/ESTADO_PROYECTO.md` sección 32.

**Próximo paso confirmado con Kevin:** abrir los 4 PRs en GitHub cuando se pida, o agregar templates de Carta Oferta de Incendio y Vida/AP (requieren texto oficial), o retomar Fase 2 (Auto) si se pide.
