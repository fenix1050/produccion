...
function detailedDescription(value) {
  const normalized = String(value ?? '')
    .trim()
    .normalize('NFD')
    .replace(/\p{Diacritic}/gu, '')
    .toLowerCase()
  return normalized && normalized !== 'no disponible' ? `<p>${text(value)}</p>` : ''
}
...
function classifiedIdentityDocuments(insured) {
  const document = String(insured.documento ?? '').trim()
  if (!document) return { documento: UNAVAILABLE, ruc: UNAVAILABLE }

  const tipoPersona = String(insured.tipo_persona ?? '')
    .trim()
    .normalize('NFD')
    .replace(/\p{Diacritic}/gu, '')
    .toLowerCase()
  const showRuc = tipoPersona === 'juridica' || document.includes('-')

  return showRuc
    ? { documento: UNAVAILABLE, ruc: document }
    : { documento: document, ruc: UNAVAILABLE }
}

function insuredPanel(insured, proposer, draft) {
  const identityDocuments = classifiedIdentityDocuments(insured)
  ...
  return `
    <section class="insured-panel">
      ...
      <div class="insured-grid">
        ...
        ${field('C.I. / Documento', identityDocuments.documento, 'span-3')}
        ${field('R.U.C.', identityDocuments.ruc, 'span-2')}
        ${field('Nacionalidad', insured.nacionalidad, 'span-3')}
        ${field('Estado civil', insured.estado_civil, 'span-2')}
        ${field('E-mail', insured.email, 'span-4')}
        ${field('Lugar de trabajo', insured.lugar_trabajo, 'span-2')}
        ${field('Ocupación', insured.ocupacion, 'span-3')}
  ...
  const pageOne = `
    <article class="proposal-page proposal-page--one">
      ...
      <section class="risk-table">
        ...
        <div class="risk-columns risk-columns--body"><span>1</span><div class="risk-description fit-box" data-fit-section="risk-description" data-fit-target="9.2" data-fit-minimum="8" data-fit-step="0.2">${detailedDescription(draft.descripcion_detallada)}<b>UBICACIÓN DEL RIESGO:</b><br />${text(risk.direccion)}${risk.ciudad ? `, ${text(risk.ciudad)}` : ''}<br /><br /><b>DETALLE DE SUMAS ASEGURADAS:</b><br />${coverageSummary(coverages)}</div><b>${money(totalCoverage)}</b><b>${money(commercial.variante?.prima)}</b></div>
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .header-meta-row--secondary { grid-template-columns: 87.25% 12.75%; }
  .header-cell { min-width: 0; display: flex; align-items: center; gap: .55mm; padding: .4mm 1mm; border-bottom: .55px solid var(--rule); line-height: 1.1; overflow: hidden; white-space: nowrap; }
  .header-meta-row--primary .header-cell { border-bottom: 0; }
  ...
  .insured-grid { display: grid; grid-template-columns: repeat(16, minmax(0, 1fr)); grid-auto-rows: 4.65mm; border: .65px solid var(--rule); background: linear-gradient(var(--rule), var(--rule)) 0 13.95mm / 100% .5px no-repeat, linear-gradient(var(--rule), var(--rule)) 0 18.6mm / 100% .5px no-repeat, linear-gradient(var(--rule), var(--rule)) 0 23.25mm / 100% .5px no-repeat; }
  .insured-field { min-width: 0; padding: .55mm .7mm; font: inherit; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; }
  ...
  .risk-columns--body .risk-description { padding-block: .1mm; }
  .risk-columns--body .risk-description p { margin: 0; }
