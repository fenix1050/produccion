...
test('PF-3 render diagnostics use only the server capability and a non-emitting POST action', async () => {
  const moduleSource = await readFile(moduleUrl, 'utf8')

  assert.match(moduleSource, /state\.usuario\?\.rol === 'admin'/)
  assert.match(moduleSource, /state\.propuesta\?\.estado === 'error_pdf'/)
  assert.match(moduleSource, /puede_probar_render_error_pdf === true/)
  assert.match(
    moduleSource,
    /api\.post\(`\/propuestas\/\$\{state\.propuesta\.id\}\/test-render-error-pdf`, \{\}\)/
  )
  assert.match(moduleSource, /state\.probePending = true/)
  assert.match(moduleSource, /data-action="probar-render-error-pdf"/)
  assert.match(moduleSource, /Diagnóstico de render completado\./)
  assert.match(moduleSource, /No fue posible completar el diagnóstico de render\./)
})
