...
function isTestRenderProbeEnabled() {
  return (
    process.env.NODE_ENV !== 'production' && process.env.PF3_TEST_RENDER_PROBE_ENABLED === 'true'
  )
}

export function puedeProbarRenderErrorPdf(propuesta, usuario) {
  return Boolean(
    isTestRenderProbeEnabled() &&
    usuario?.rol === 'admin' &&
    propuesta?.estado === 'error_pdf' &&
    propuesta.snapshot_json
  )
}

export async function probarRenderErrorPdf(id, usuario) {
  if (!isTestRenderProbeEnabled()) {
    throw httpError(404, 'Recurso no encontrado')
  }
  if (usuario.rol !== 'admin') {
    throw httpError(403, 'No tenés permiso para probar el render de la Propuesta Formal')
  }

  const propuesta = await propuestasRepository.findPropuestaContextById(id)
  if (propuesta.estado !== 'error_pdf') {
    throw httpError(409, 'La Propuesta Formal no está disponible para una prueba de render')
  }
  if (!propuesta.snapshot_json) {
    throw httpError(409, 'La Propuesta Formal no tiene un snapshot disponible para renderizar')
  }

  try {
    await renderPropuestaMrcPdf(propuesta.snapshot_json)
  } catch {
    throw httpError(500, 'No fue posible generar el PDF de prueba')
  }
}
