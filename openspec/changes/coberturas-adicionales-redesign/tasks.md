...
- [ ] 5.3 Confirm live panel prima / "Capital total asegurado" updates after typing amount (calculation path unaffected) — **NOT fully verified live**: the test form was never filled with all MRC-required fields (tipo de riesgo, ciudad, capitales) in any session, so `puedeAvanzarADetalle()`/full preview never completed. Low risk: the calculation code path (`armarRiesgoDatosMrc`, `state.coberturasAdicionales[].sumaAsegurada`) has zero diff in this change (display-only rewrite) — this task remains open as a known, low-priority gap, not a blocker.
- [x] 5.4 Confirm zero new console errors, both roles, all 3 widths — checkbox role confirmed (only pre-existing benign 401 on `/auth/me` before login resolves); free-selector role confirmed 2026-08-11 (same pre-existing 401s plus one pre-existing, unrelated 404 for the documented-missing `frontend/shared/config.js`, not introduced by this change)
- [ ] 5.5 Confirm read-only `.cobertura-card` in "Detalle del plan" unaffected (`SUBLIMITE_ICONOS` byte-identical, no `render-detalle-plan.js` diff) — **NOT visually verified live** (same reason as 5.3: "Detalle del plan" tab never unlocked in any session). `render-detalle-plan.js` has zero diff across all 3 merged PRs (confirmed via `git diff --stat` at apply time), so regression risk is effectively nil — this task remains open as a known, low-priority gap, not a blocker.
