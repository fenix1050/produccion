...
BEGIN
  ...
  IF FOUND THEN
    ...
    ELSE
      RETURN QUERY SELECT v_carta.id, v_carta.version, v_carta.estado::TEXT, v_carta.pdf_storage_path, v_carta.pdf_hash::TEXT, FALSE, TRUE, v_carta.snapshot_json;
    END IF;
    RETURN;
  END IF;
  ...
  INSERT INTO cartas_oferta (
    ...
    (
      ...
      FROM cartas_oferta
      WHERE cotizacion_id = p_cotizacion_id
        AND cartas_oferta.estado = 'reemplazada'
      ORDER BY cartas_oferta.version DESC
      LIMIT 1
    ),
...
-- ---------------------------------------------------------------------------
-- Explicit least-privilege ACLs for Carta Oferta / Propuesta RPCs.
-- These statements intentionally preserve service_role as the only API role
-- allowed to execute these RPCs.
-- ---------------------------------------------------------------------------

REVOKE ALL ON FUNCTION public.iniciar_carta_oferta_generacion(
    integer, text, jsonb, text, text, text, text, integer, jsonb
) FROM PUBLIC, authenticated, anon, service_role;

GRANT EXECUTE ON FUNCTION public.iniciar_carta_oferta_generacion(
    integer, text, jsonb, text, text, text, text, integer, jsonb
) TO service_role;


REVOKE ALL ON FUNCTION public.emitir_carta_oferta(
    bigint, text, text, integer
) FROM PUBLIC, authenticated, anon, service_role;

GRANT EXECUTE ON FUNCTION public.emitir_carta_oferta(
    bigint, text, text, integer
) TO service_role;


REVOKE ALL ON FUNCTION public.registrar_error_carta_oferta(
    bigint, text
) FROM PUBLIC, authenticated, anon, service_role;

GRANT EXECUTE ON FUNCTION public.registrar_error_carta_oferta(
    bigint, text
) TO service_role;
...
REVOKE ALL ON FUNCTION public.motivo_ineligibilidad_carta_propuesta(
    bigint, integer, boolean
) FROM PUBLIC, authenticated, anon, service_role;

GRANT EXECUTE ON FUNCTION public.motivo_ineligibilidad_carta_propuesta(
    bigint, integer, boolean
) TO service_role;


REVOKE ALL ON FUNCTION public.crear_o_recuperar_propuesta_borrador(
    bigint, integer, boolean
) FROM PUBLIC, authenticated, anon, service_role;

GRANT EXECUTE ON FUNCTION public.crear_o_recuperar_propuesta_borrador(
    bigint, integer, boolean
) TO service_role;


REVOKE ALL ON FUNCTION public.actualizar_propuesta_borrador(
    bigint, integer, integer, integer, jsonb, integer, boolean
) FROM PUBLIC, authenticated, anon, service_role;

GRANT EXECUTE ON FUNCTION public.actualizar_propuesta_borrador(
    bigint, integer, integer, integer, jsonb, integer, boolean
) TO service_role;


-- Tables: service_role may SELECT / INSERT / UPDATE, but not DELETE or
-- any other table privilege.

REVOKE ALL ON TABLE
    public.cartas_oferta,
    public.propuestas_formales
FROM PUBLIC, authenticated, anon, service_role;
...
-- Sequences: service_role requires USAGE only.

REVOKE ALL ON SEQUENCE
    public.cartas_oferta_id_seq,
    public.propuestas_formales_id_seq
FROM PUBLIC, authenticated, anon, service_role;
