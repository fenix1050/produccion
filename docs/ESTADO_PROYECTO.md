...
**PR 1 — backend, rama `sdd/mrc-plan-descuento-fijo-backend` (commit `e60a35b`, NO pusheada todavía).** Objetivo: un nuevo plan MRC con un descuento comercial fijo del 10% (política de empresa, no discrecionalidad del agente), editable solo por roles con el permiso nuevo `puede_editar_descuento_plan`.

- Migración `046_plan_mrc_descuento_fijo.sql` (committeada, **NO aplicada contra Supabase real** — pendiente de que Kevin confirme el nombre exacto del plan, hoy placeholder): agrega `roles.puede_editar_descuento_plan` (default `FALSE`, `TRUE` para `admin`), inserta el plan MRC nuevo con `descuento_default = 10`, `descuento_maximo = 10`, `cotizacion_combinada = FALSE`, y sus `plan_formas_pago` (solo Contado habilitado).
...
**Pendiente antes de cerrar el cambio**: verificación manual en vivo con el skill `run-cotizador` (login sin permiso → campo bloqueado en 10% → prima refleja 10%; otorgar el permiso vía admin Roles → re-login → campo editable), aplicar la migración 046 contra Supabase real una vez confirmado el nombre del plan, y luego `sdd-verify`/archivo formal. Ninguna de las dos ramas está pusheada ni tiene PR abierto todavía — las abre Kevin.
